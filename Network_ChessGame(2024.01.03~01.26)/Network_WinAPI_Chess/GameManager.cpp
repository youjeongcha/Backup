...
GameManager::GameManager()
{
	...
	m_PlayerTurn = CAMP_WHITE; //서브 화면 흰색으로 하기 위해
	//isWaitlayer = true;
	//isDelay = true;
...
{
	m_PlayerTurn = playerTeam; ///  TODO::서브 창에 색상 뜨는지 확인
	m_PalyerState = WAIT_PLAYER;
}
...
void GameManager::DrawTurnState(HDC hdc_Sub)
{
	...
	SetBkMode(hdc_Sub, TRANSPARENT); //글자 뒷배경 투명화
	//SetBkColor(hdc, RGB(0, 0, 0));
	SetTextColor(hdc_Sub, RGB(128, 128, 128)); //글자 색 변경
