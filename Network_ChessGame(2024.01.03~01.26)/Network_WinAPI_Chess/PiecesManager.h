...
private:
	...
	PROMOTION_RECT m_PromotionType; //말이 어떤 종류의 말로 승진할지 판단하기 위해
...
public:
	...
	//rect가 이동한 후의 위치
	void MovePiece_FromServer(int _x, int _y, RECT _moveRect);

	//말 이동 데이터가 맨 끝 줄에 닿았는지 확인
	bool IsDataMoveEnd(CAMP team, int y);


	//승진 말
	Piece* GetErasePawn() { return this->m_ErasePawn; }
	void SetErasePawn(Piece* _ErasePawn) { this->m_ErasePawn = _ErasePawn; }


	//승진할 말의 >> 승진 종류 기록
	PROMOTION_RECT Get_Promotion()
	{
		return this->m_PromotionType;
	}

	//승진 진행
	void Promotion(PROMOTION_RECT _PromotionType);
};
