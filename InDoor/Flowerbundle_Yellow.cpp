...
Flowerbundle_Yellow::Flowerbundle_Yellow()
{
    Initialize();
}

Flowerbundle_Yellow::Flowerbundle_Yellow(const ObjectData& dataSet, int index) : Object(dataSet, index)
{
    Initialize();
}
...
VOID Flowerbundle_Yellow::Initialize()
{
    growthRate = FRUITS;
    renderer->SetSrc(2, 0);
}

VOID Flowerbundle_Yellow::Move(const FLOAT& deltaTime)
{
    return VOID();
}

void Flowerbundle_Yellow::PassOfthe_TimeChangeBitmap()
{
...
VOID Flowerbundle_Yellow::Update(const FLOAT& deltaTime)
{
...
void Flowerbundle_Yellow::DetailSelect(int selectNum)
{//0~선택지 개수-1의 리스너를 바인딩하기 위해 리턴한다. 
    ...
    case 0:
        if (growthRate == SOIL)
        {
            ...
            {
                //아이템 사용
                if (!GameMgr->MinusPlayerItem(SEED_YELLOW))
                {
    ...
    case 2:
        if (growthRate == FRUITS)
        {
            ...
            GameMgr->PlusPlayerInventory({ SEED_YELLOW, 1 }); //씨앗
            GameMgr->PlusPlayerInventory({ FRUIT_YELLOW, 1 }); //열매
            //흙으로 초기화
            growthRate = SOIL;
            renderer->SetSrc(0, 0);
            seedPlanting = false;
